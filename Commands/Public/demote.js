...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('demote')
        .setDescription('Demote a ROBLOX player one rank down')
    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            // Preview next rank (demote)
            let nextRoleName = "Previous Rank";
            try {
                const previewRes = await axios.post(proxyUrl, {
                    userId: String(userId),
                    action: "preview_demote",
                    groupId: groupId
                }, {
            ...
            const confirmEmbed = new EmbedBuilder()
                ...
                .setDescription(`Are you sure you want to demote this player?`)
                .addFields(
            ...
            collector.on('collect', async i => {
                ...
                if (i.customId === 'cancel_rank') {
                    await i.update({ content: "Demotion cancelled.", embeds: [], components: [] });
                    collector.stop();
                    return;
                }

                if (i.customId === 'confirm_rank') {
                    await i.update({ content: "Demotion in progress...", embeds: [], components: [] });
                    ...
                    if (response.data.success) {
                        const newRoleName = response.data.newRoleName || nextRoleName || "One rank lower";
